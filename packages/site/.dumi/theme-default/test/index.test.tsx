...
describe('default theme', () => {
  ...
  it('should render site home page', () => {
    ...
    expect(
      queryByAttribute('data-mobile-show', container, 'true'),
    ).not.toBeNull();

    // expect dark render and click success
    const menu = queryByAttribute('class', container, '__dumi-default-menu');
    const sunMenu = queryByAttribute(
      'class',
      menu,
      '__dumi-default-dark-sun __dumi-default-dark-switch-active',
    );
    expect(sunMenu).not.toBeNull();
    const moonMenu = queryByAttribute(
      'class',
      container,
    ...
    const navbar = queryByAttribute(
      'class',
      container,
      '__dumi-default-navbar',
    );
    const moonNav = queryByAttribute(
      'class',
      navbar,
      '__dumi-default-dark-moon __dumi-default-dark-switch-active',
    );
    moonNav.click();
    expect(
      queryByAttribute(
        'class',
        navbar,
        '__dumi-default-dark-switch __dumi-default-dark-switch-open',
      ),
    ).not.toBeNull();
    const switchList = queryByAttribute(
      'class',
      navbar,
    ...
    expect(
      queryByAttribute('class', navbar, '__dumi-default-dark-switch-list'),
    ).toBeNull();
    expect(
      queryByAttribute(
        'class',
  ...
  it('should render documentation page', async () => {
    ...
    expect(
      new Date(updatedTime).toLocaleString([], { hour12: false }),
    ).not.toBeNull();
  ...
  it('should render builtin components correctly', () => {
    ...
    expect(
      queryAllByAttribute(
        'class',
        container,
        '__dumi-site-tree-icon icon-minus-square',
      ).length,
    ).toBe(2);
    expect(
      queryAllByAttribute(
        'class',
        container,
        '__dumi-site-tree-icon icon-folder-open',
      ).length,
    ).toBe(2);
    expect(
      queryAllByAttribute('class', container, '__dumi-site-tree-icon icon-file')
        .length,
    ).toBe(1);
    ...
    expect(
      queryAllByAttribute(
        'class',
        container,
        '__dumi-site-tree-icon icon-plus-square',
      ).length,
    ).toBe(1);
    expect(
      queryAllByAttribute(
        'class',
        container,
        '__dumi-site-tree-icon icon-folder',
      ).length,
    ).toBe(1);
    ...
    (
      container.querySelector(
        '[data-iframe] button[role=refresh]',
      ) as HTMLElement
    ).click();
    expect(container.querySelector('[data-iframe]').innerHTML).not.toContain(
      'demo-3 Content',
    );
    expect(container.querySelector('[data-iframe] iframe')).not.toBeNull();
    expect(
      (container.querySelector('[data-iframe] iframe') as HTMLElement).style
        .height,
    ).toEqual('100px');
